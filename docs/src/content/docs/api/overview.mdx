...
<CardGrid>
  <Card title="Behind Caddy Reverse Proxy" icon="rocket">
    - **CAPF API**: `https://{domain}/api/capf/api/v1/`
    - **TVS API**: `https://{domain}/api/tvs/api/v1/`
  </Card>

  <Card title="Direct Access (Internal)" icon="laptop">
...
<Aside type="danger">
Currently, the APIs do not require authentication. **In production, you must secure these endpoints!**
</Aside>
...
- <Badge text="201" variant="success" /> - Created
- <Badge text="204" variant="success" /> - No Content (successful deletion)
- <Badge text="400" variant="caution" /> - Bad Request (validation error)
- <Badge text="404" variant="caution" /> - Not Found
- <Badge text="500" variant="danger" /> - Internal Server Error
...
<CardGrid>
  ...
  <Card title="Security" icon="seti:lock">
    ...
    **Endpoints**:
    ...
    [Full Security API Reference →](./security)
  </Card>

  <Card title="System" icon="information">
